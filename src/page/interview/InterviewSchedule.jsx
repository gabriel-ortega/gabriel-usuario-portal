...
export default function InterviewSchedule() {
  ...
  const [schedule,setSchedule] = useState({data:[],value:false})
  ...
  const loadResults = async () => {
    ...
    console.log(interviewStartDate)
   
      // Compara la fecha de inicio con la fecha actual
      return item.interviewee == userData.uid && interviewStartDate >= currentDate;
    });
...
dispatch(getUserData(userData.uid))
  ...
  const handleDateClick = (day) => {
    ...
    const filtered = dates.filter((event) => {
      const eventDate =   new Date(event.start).setHours(0, 0, 0, 0) == normalizeDate(format(day, "yyyy-MM-dd")).setHours(0, 0, 0, 0)   ; // Convertir el campo start a objeto Date
  ...
  const assignRandomInterviewee = (hourGroup) => {
   ...
   /*  console.log(selectedEvent) */
    ...
    setDatesCurrent((prevDates) => {
      ...
      setSchedule(
        {data:selectedEvent,
        value:true})// Pasa los datos actualizados aquí
      loadResults()
  ...
  useEffect(() => {
    ...
    if (schedule.data && schedule.data.length > 0) {
      const interval = setInterval(() => {
        const currentDate = new Date();
        const targetDate = new Date(schedule.data[0].start); // Usando el primer evento de schedule.data
        ...
        if (diff <= 0) {
          ...
          setTimeLeft(`${hours} horas, ${minutes} minutos y ${seconds} segundos`);
        }
  ...
  return (
    ...
    <h1 className='text-center text-2xl font-semibold'>Appointment calendar</h1>
    {schedule.value?<div>
      <button type="button" onClick={handleCancelSchedule} className=' rounded-md bg-red-500 w-44 mt-10 h-10 text-white text-balance'>Cancel Interview</button>
      ...
      <div className="bg-white p-2 rounded-lg shadow-lg text-center w-full max-w-md">
        <h2 className="text-3xl font-semibold text-gray-800 mb-4">Time remaining for interview:</h2>
        <p className="text-xl font-medium text-blue-600 mb-4">{timeLeft}</p>
        <div className="flex justify-center space-x-4">
          <div className="flex flex-col items-center">
          <span className="text-5xl font-bold text-gray-800">
  {schedule.data && schedule.data[0] && schedule.data[0].start
    ? new Date(schedule.data[0].start).toLocaleDateString()
    : ""}
</span>
<span className="text-xl text-gray-500">
  {schedule.data && schedule.data[0] && schedule.data[0].start
    ? new Date(schedule.data[0].start).toLocaleTimeString()
    : "00:00:00"}
</span>
            <span className="text-lg text-gray-500">Date and time of interview</span>
          </div>
        </div>
      </div>
    </div>
    <p className='text-center font-semibold text-lg text-balance'>The following items are recommended for the day of the interview</p>
...
<>
    <p className='text-lg pt-6 pb-5'>Welcome, choose a date and time to schedule your appointment.</p>
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-0 justify-center items-center">
      ...
      <div className="bg-blue-300 p-6 rounded-lg shadow-md">
        <div className="flex justify-between items-center mb-4">
          ...
          <h2 className="text-xl font-bold text-gray-50">{format(currentMonth, "MMMM yyyy")}</h2>
          <button
    ...
    <div>
      ...
      <p className='text-lg text-center font-semibold pb-5'>Hours Available</p>
    ...
    ))} </div>: <p>No hours were found available for this date</p> }
